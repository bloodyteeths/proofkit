#!/usr/bin/env python3
"""ProofKit Certificate Generator - Final Fixed Version with all patches applied."""
...
def load_logo():
    """PATCH 1: Strictly require the SVG - crash if missing."""
    svg_path = Path("assets/proofkit_logo_icon.svg")
    if not svg_path.exists():
        raise RuntimeError(f"Logo not found at {svg_path}")
    logo_drawing = svg2rlg(str(svg_path))
    if not logo_drawing:
        raise RuntimeError(f"Failed to parse SVG at {svg_path}")
    return logo_drawing
...
def paint_static(canvas_obj, doc, logo_drawing):
    ...
    # PATCH 2: Watermark (real logo, 5% opacity, 120mm dia.)
    scale = (120*mm) / max(logo_drawing.width, logo_drawing.height)
    canvas_obj.saveState()
    canvas_obj.setFillAlpha(0.05)
    canvas_obj.setStrokeAlpha(0.05)
    canvas_obj.translate(PAGE_WIDTH/2 - (logo_drawing.width*scale)/2,
                         PAGE_HEIGHT/2 - (logo_drawing.height*scale)/2)
    canvas_obj.scale(scale, scale)
    renderPDF.draw(logo_drawing, canvas_obj, 0, 0)
...
class CertificateGenerator:
    ...
    def create_header(self, certificate_no: str) -> list:
        ...
        title_style = ParagraphStyle(
            'Title',
            fontName='CormorantGaramondSC-ExtraBold',
            fontSize=18,
    ...
    def create_status_badge(self, is_pass: bool) -> Table:
        ...
        badge_table.setStyle(TableStyle([
            ...
            ('FONTNAME', (0, 0), (0, 0), 'CormorantGaramondSC-ExtraBold'),
            ('FONTSIZE', (0, 0), (0, 0), 18),
            ('ROUNDEDCORNERS', [5]),
        ]))
    ...
    def create_verification_section(self, verification_hash: str) -> Table:
        ...
        qr_caption = Paragraph('Scan to verify',
                              ParagraphStyle('Caption', fontName='Inter',
    ...
    def create_signature_section(self) -> Table:
        ...
        # Add logo to seal (properly rendered)
    ...
    def generate_certificate(self, spec: dict, decision: dict, 
        ...
        try:
            ...
            # PATCH 4: Build content in correct order
            elements = []
            ...
            # PATCH 3: Temperature profile (if provided) - force new page if needed
            if plot_path and os.path.exists(plot_path):
                try:
                    # Add page break to ensure graph gets its own page
                    ...
                    # PATCH 3: Shrink plot to fit: 150 × 70 mm
